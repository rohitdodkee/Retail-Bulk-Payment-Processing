...
export default function HomePage () {
  ...
  const deleteTable = async (templateId) => {
    
    try {
      await axios.delete(`http://localhost:9000/api/template/delete/${templateId}`);
      const newData = apiData.filter((d) => d.template_id !== templateId);
      setApiData(newData);
      console.log("Deleting table with ID", templateId);
